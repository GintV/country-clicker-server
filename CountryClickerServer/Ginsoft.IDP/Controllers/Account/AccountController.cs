...
namespace Ginsoft.IDP.Controllers.Account
{
    ...
    public class AccountController : Controller
    {
        ...
        public AccountController(
            ...
            IGinsoftUserRepository ginsoftUserRepository)
        {
        ...
        public async Task<IActionResult> Login(LoginInputModel model, string button)
        {
            ...
            {
                if (GinsoftUserRepository.AreUserCredentialsValid(model.Username, model.Password))
                {
                    var user = GinsoftUserRepository.GetUserByUsername(model.Username);
        ...
        public async Task<IActionResult> ExternalLoginCallback()
        {
            ...
            var user = GinsoftUserRepository.GetUserByProvider(provider, userId);
            if (user == null)
            {
                ...
                if (provider == "Facebook")
                {
                    ...
                    if (email != null)
                    {
                        var userByEmail = GinsoftUserRepository.GetUserByEmail(email.Value);
                        if (userByEmail != null)
                        {
                            // add Facebook as a provider for this user
                            GinsoftUserRepository.AddUserLogin(userByEmail.SubjectId, provider, userId);

                            if (!GinsoftUserRepository.Save())
                            {
